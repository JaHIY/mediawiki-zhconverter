...
convert_chinese() {
    ...
    shift
    printf '%b' $(sed -e 's/</\&lt;/g' -e 's/>/\&gt;/g' "$@" | awk "$AWK_SYNTAX" | curl -s -d "${MEDIAWIKI_DATA}${MEDIAWIKI_USELANG}" --data-urlencode "text@-" "$MEDIAWIKI_API" | sed -e 's/&lt;/</g' -e 's/&gt;/>/g' -e "$MEDIAWIKI_RETURN_SUBSTITUTE") | sed -e 's;\\\/;/;g'
}
